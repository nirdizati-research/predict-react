...
function getIntermediateResults(array, interResults) {
    ...
    array.map(function (trace) {
        if (trace.n_events === 3) {
            // eslint-disable-next-line
            interResults = addToSet(interResults, {
                id: trace.id, duration: (parseInt(trace.reg_results) + parseInt(trace.duration)),
                class_results: trace.class_results, timeSeriesPred_results: trace.timeSeriesPred_results
            });
        }
    });
    return interResults;
